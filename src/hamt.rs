...
impl<K: Hash + PartialEq + Clone, V: Clone> HAMT<K, V> {
    pub fn new() -> HAMT<K, V> {
        ...
        HAMT {
            ...
            root: Hp::init(RootedNode(0, Vec::with_capacity(1))),
        }
    }
                ...
                let (new_node, cont_search) = match *cur_node {
                    ...
                    ref node @ RootedNode(..) => {
                        let (map, arr) = match *node {
                            MappedNode(ref map, ref arr) |
                            RootedNode(ref map, ref arr) => (map, arr),
                            _ => unreachable!(),
                        };
                        match state.compressed_index(map) {
                            Ok(idx) => {
                            ...
                            Err(idx) => {
                                ...
                                match *node {
                                    MappedNode(..) => (MappedNode(bitmap, arrmap), false),
                                    _ => (RootedNode(bitmap, arrmap), false),
                                }
                            }
                        }
                    }
                    ...
                    DeletedMap(ref map, ref arr) => {
                        self.repair_deleted_node(&mut pre_node,
                                                 state.prev(),
                                                 if arr.len() > 0 {
                                                     Some((map, arr))
                                                 } else {
                ...
                match sstate.compressed_index(map) {
                    Ok(idx) => {
                        ...
                        match insert {
                            ...
                            None => bitmap.set(sstate.current_uncompressed_index(), false),
                        };
...
impl<'a, K: Hash + 'a> SearchState<'a, K> {
    ...
    fn prev<'b>(&'b mut self) -> &'b mut SearchState<'a, K> {
        ...
        if self.curr_off == 0 {
            ...
            self.hash_gen = SipHasher::new_with_keys(self.hash_base,
                                                     (self.curr_lvl - lvls_per_hash) as u64);
            // self.curr_hsh = (self.hash_gen).hash(self.curr_key);
            self.hash_update();
    ...
    fn compressed_index(&mut self, bitmap: &usize) -> Result<usize, usize> {
        let uncompressed_index = ((self.curr_hsh >> self.curr_off) as usize) & INDEX_MAP;
    ...
    fn mixed_ins_remove_parallel() {
        ...
        for x in 0..NUM_THREADS {
            ...
            guards.push(thread::spawn(move || {
                for i in (NUM_ELEMENTS * x)..NUM_ELEMENTS * (x + 1) {
                    lamt.remove(i);
                    lamt.insert(i + (NUM_THREADS * NUM_ELEMENTS),
                                i + (NUM_THREADS * NUM_ELEMENTS));
                }
                c.wait();
            }));
        }
